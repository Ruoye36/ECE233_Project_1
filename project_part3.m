...
for i_nrf=1:length(N_RF_list)
    ...
    % satisfies "if the number of RF chains is twice the total number of data streams, the
    % hybrid beamforming structure can realize any fully digital beamformer exactly"

    % Initialize Tx precoder and combiner
    ...
    for i_snr=1:length(SNR)
        ...
        for i_mc=1:num_mc
            ...
            [Left,Delta,U_e]=svd(Heff*pinv(sqrtm(Q)));  % TODO U_e' ?
            ...
            lambda_list(Ns+1:end,:)=[];
            lambda_quant_list=diag(Delta_quant);
            lambda_quant_list(Ns+1:end,:)=[];
            lambda_sq_list=lambda_list.^2;
            lambda_quant_sq_list=lambda_quant_list.^2;
            
            
            %Nmin=min(Nt_RF,Nr_RF);  % NOTE  # RF chains; here should be # antennas N,M
            % NOTE water-filling solution: refer to MIMO.pdf formula 7.11 and above
            % NOTE assume Nmin (mu?) interval (0, /2, /2, /2, ...)
            % to obtain Nmin
            %for Nmin=1:maxval
            Nmin=1;
            mu=0;
            while Nmin<Ns && ~(mu>=sigma^2/lambda_sq_list(Nmin) && mu<=sigma^2/lambda_sq_list(Nmin+1))   % TODO >= <= or > < ?
                Nmin=Nmin+1;
                mu=(P+sum(sigma^2./lambda_sq_list(1:Nmin)))/Nmin;   % TODO ./!!!
            end
            Nmin_quant=1;
            mu_quant=0;
            while Nmin_quant<Ns && ~(mu_quant>=sigma^2/lambda_sq_list(Nmin_quant) && mu_quant<=sigma^2/lambda_sq_list(Nmin_quant+1))   % TODO >= <= or > < ?
                Nmin_quant=Nmin_quant+1;
                mu_quant=(P+sum(sigma^2./lambda_quant_sq_list(1:Nmin_quant)))/Nmin_quant;   % TODO ./!!!
            end
            P_star_list=max(mu-N0./lambda_sq_list,0);
            P_star_list(Nmin+1:end)=[];
            P_star_quant_list=max(mu_quant-N0./lambda_quant_sq_list,0);
            P_star_quant_list(Nmin_quant+1:end)=[];

            Gamma_e=diag([transpose(sqrt(P_star_list)) zeros(1,N_RF-Nmin)]);
            Gamma_e_quant=diag([transpose(sqrt(P_star_quant_list)) zeros(1,N_RF-Nmin_quant)]);
            ...
            %% Part 3: IV.C Hybrid Combining Design for N_RF=NS
